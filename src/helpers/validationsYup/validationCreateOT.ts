...
export const validationCreateOT = yup.object().shape({
  ...
  numero_de_orden_de_compra: yup
    .string()
    .min(3, 'El numero de orden de compra debe de tener mas de 3 caracteres'),
  ...
  tiempoDeEjecucion: yup
    ...
    .min(
      1,
      'El tiempo de ejecución tiene que ser un número y no puede ser negativo',
    ),
  fecha_cierre: yup.date().required('Campo requerido.'),
  imgDeLaMaquina: yup
    .string()
    .required('Campo requerido')
    .min(1, 'mínimo 1 imagen por ot'),
  tareas: yup
    .string()
    .required('Campo requerido.')
    .min(3, 'Las tareas deben tener más de 2 caracteres'),
  comentario: yup
    .string()
