...
export const validationCreateFollow = yup.object().shape({
  imgDeVerificacion: yup
    .string()
    .required('Campo requerido.')
    .min(1, 'mínimo 1 imagen por seguimiento'),
  comentario: yup
    .string()
    .required('Campo requerido.')
  ...
  tiempoDeFuncionamiento: yup
    ...
    .min(
      1,
      'El tiempo de funcionamiento tiene que ser un número y no puede ser negativo',
    ),
  tiempoDeReparacion: yup
    ...
    .min(
      0,
      'El tiempo de reparación tiene que ser un número y no puede ser negativo',
    ),
  ...
  tiempoDeFalla: yup.number().when('presentaFalla', {
    ...
    then: yup
      ...
      .min(
        0,
        'El tiempo de falla tiene que ser un número y no puede ser negativo',
      ),
  }),
});
